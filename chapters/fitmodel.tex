...
To start consider a binned histogram containing a distribution from a kinematic variable---like the mass of the parent particle---that we want to use in the hypothesis test. 
...
\item {To form the test statistic, the \textit{profile likelihood ratio} is used 
  ...
  Where $\hat{\mu}$ and $\hat{\theta}$ are the maximum likelihood estimators, and $\hat{\hat{\theta}}$ the optimized value of the estimator. 
  }
